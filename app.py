...
#         return None
def save_plot(fig):
    """Save Plotly figure to PNG using kaleido."""
    if fig is None:
        return None
      
    try:
        # 1. Check if the object is a Plotly figure (has the write_image method)
        if hasattr(fig, 'write_image'):
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"outputs/plot_{timestamp}.png"
            
            # 2. Use Plotly's write_image method. kaleido handles the backend conversion.
            # We explicitly define width/height for high-quality export.
            fig.write_image(filename, width=1200, height=800) 
            
            # 3. Clean up matplotlib figure resources if any were created (safer to remove this)
            # You can remove the matplotlib import/calls entirely since you are using Plotly.
            
            return filename
        else:
            # This handles cases where the state might hold something unexpected
            print(f"Warning: Object passed to save_plot does not have write_image method. Type: {type(fig)}")
            return None
            
    except Exception as e:
        print(f"Error saving plot: {e}")
        # Print the full error traceback to help diagnose kaleido issues
        import traceback
        traceback.print_exc()
        # Optional: Provide a user-friendly error path
        # raise gr.Error("PNG export failed. Check server logs for kaleido errors.")
        return None

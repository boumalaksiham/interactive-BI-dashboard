...
def create_heatmap_plot(df):
    ...
    try:
        fig = viz_manager.create_visualization("heatmap", df, title="Correlation Heatmap")
        return fig
    except Exception as e:
...
# ============================================================================
# DROPDOWN UPDATE HELPERS (NO df_state.change)
# ============================================================================


def update_filter_dropdowns(df):
    """Update filter tab dropdowns when a new dataset is loaded."""
    if df is None:
        return (
            gr.Dropdown(choices=[]),
            gr.Dropdown(choices=[]),
    ...
    return (
        gr.Dropdown(choices=num_cols),
        gr.Dropdown(choices=cat_cols),
        gr.Dropdown(choices=date_cols),
        f"**Current Rows:** {len(df):,} (unfiltered)",
    )


def update_viz_dropdowns(df):
    """Update visualization tab dropdowns when a new dataset is loaded."""
    if df is None:
        empty = gr.Dropdown(choices=[])
        empty_none = gr.Dropdown(choices=["None"])
        return empty, empty, empty, empty_none, empty, empty, empty_none, empty
    ...
    return (
        gr.Dropdown(choices=date_cols),         # ts_date_col
        gr.Dropdown(choices=num_cols),          # ts_value_col
        gr.Dropdown(choices=num_cols + cat_cols),  # dist_col
        gr.Dropdown(choices=cat_cols),          # cat_col
        gr.Dropdown(choices=["None"] + num_cols),   # cat_value_col
        gr.Dropdown(choices=num_cols),          # scatter_x
        gr.Dropdown(choices=num_cols),          # scatter_y
        gr.Dropdown(choices=["None"] + df.columns.tolist()),  # scatter_color
    )


def update_export_dropdowns(df):
    """Update export tab dropdowns when a new dataset is loaded."""
    if df is None:
        return gr.Dropdown(choices=[]), gr.Dropdown(choices=[])

    date_cols = df.select_dtypes(include=["datetime64[ns]", "datetime64"]).columns.tolist()
    return gr.Dropdown(choices=df.columns.tolist()), gr.Dropdown(choices=date_cols)
...
def create_dashboard():
    ...
    with gr.Blocks(title="Business Intelligence Dashboard") as demo:
        ...
        with gr.Tab("Filter & Explore"):
            ...
            def update_cat_values(df, col):
                ...
                return gr.CheckboxGroup(
                    choices=df[col].dropna().unique().tolist()[:50],
                    value=[],
                )
        ...
        with gr.Tab("Export"):
            ...
            with gr.Row():
                export_viz_btn = gr.Button(
                    "Export as PNG", variant="secondary"
                )
        ...
        # ====================================================================
        # WIRE UPLOAD BUTTON AFTER ALL COMPONENTS EXIST
        # ====================================================================

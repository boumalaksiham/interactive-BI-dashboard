...
#             return filename
#         else:
#             # If it's already a PlotData/dict from Gradio, we need to recreate the plot
#             # This shouldn't happen with our new design, but handle it just in case
...
def apply_date_filter(df, column, start_date, end_date):
    ...
    try:
        ...
        return df, "Error: Invalid date format. Use YYYY-MM-DD"
...
def create_dashboard():
    ...
    with gr.Blocks(title="Business Intelligence Dashboard") as demo:
        ...
        with gr.Tab("Statistics"):
            ...
            with gr.Tab("Numerical Statistics"):
                gr.Markdown(
                    "Statistics for numerical columns (count, mean, std, min, max, quartiles)"
                )
                numerical_stats_table = gr.Dataframe(
                    label="Numerical Column Statistics",
                    interactive=False,
            ...
            with gr.Tab("Categorical Statistics"):
                gr.Markdown(
                    "Statistics for categorical columns (unique values, most common, frequencies)"
                )
                categorical_stats_table = gr.Dataframe(
                    label="Categorical Column Statistics",
                    interactive=False,
        ...
        with gr.Tab("Filter & Explore"):
            ...
            # Real-time row count display
            row_count_display = gr.Markdown(
                value="**Current Rows:** Load data to begin filtering"
            )
            filter_status = gr.Markdown(value="")
                ...
                with gr.Column():
                    ...
                    with gr.Row():
                        date_start = gr.Textbox(
                            label="Start Date (YYYY-MM-DD)", placeholder="2024-01-01"
                        )
                        date_end = gr.Textbox(
                            label="End Date (YYYY-MM-DD)", placeholder="2024-12-31"
                        )
                    apply_date_filter_btn = gr.Button("Apply Date Filter")
            ...
            def update_cat_values(df, col):
                ...
                values = df[col].dropna().unique().tolist()[:50]  # Limit to 50 values
            ...
            apply_num_filter_btn.click(
                ...
                fn=lambda df: (
                    df.head(100) if df is not None else pd.DataFrame(),
                    f"**Current Rows:** {len(df):,} (filtered)"
                    if df is not None
                    else "**Current Rows:** 0",
                ),
                inputs=[filtered_df_state],
            ...
            reset_filter_btn.click(
                fn=lambda df: (
                    df,
                    f"Reset to {len(df):,} rows" if df is not None else "No data",
                    f"**Current Rows:** {len(df):,} (unfiltered)"
                    if df is not None
                    else "**Current Rows:** No data",
                ),
                inputs=[df_state],
                outputs=[filtered_df_state, filter_status, row_count_display],
            ).then(
        ...
        with gr.Tab("Visualizations"):
            ...
            with gr.Tab("Time Series"):
                ...
                with gr.Row():
                    ts_date_col = gr.Dropdown(
                        label="Date Column",
                        choices=[],
                        interactive=True,
                        allow_custom_value=False,
                    )
                    ts_value_col = gr.Dropdown(
                        label="Value Column",
                        choices=[],
                        interactive=True,
                        allow_custom_value=False,
            ...
            with gr.Tab("Distribution"):
                ...
                dist_plot_btn = gr.Button(
                    "Create Distribution Plot", variant="primary"
                )
                dist_plot = gr.Plot(label="Distribution Plot")
        ...
        with gr.Tab("Export"):
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    # Column selectors (will be populated when data is loaded)
                    export_column = gr.Dropdown(
                        label="Select Column", choices=[], interactive=True
                    )
                    export_date_col = gr.Dropdown(
                        label="Date Column (for Time Series)",
                        choices=[],
                        interactive=True,
                        visible=False,
                    )
                    ...
                    # Show/hide date column based on viz type
                    def update_column_visibility(viz_type):
                        return gr.update(visible=(viz_type == "Time Series"))
                    ...
                    create_export_viz_btn = gr.Button(
            ...
            with gr.Row():
                export_viz_btn = gr.Button(
                    "Export as PNG", variant="secondary", size="lg"
                )
                export_viz_file = gr.File(label="Download Visualization")
            ...
            def create_export_visualization(df, viz_type, column, date_col):
                ...
                try:
                    ...
                    if viz_type == "Distribution":
                        ...
                        if not corr.empty:
                            fig = viz_manager.create_visualization(
                                "heatmap", df, title="Correlation Matrix"
                            )
        ...
        # ====================================================================
        # HELPERS FOR DROPDOWN UPDATES AFTER UPLOAD
        # ====================================================================

        def update_filter_dropdowns(df):
            """Update filter dropdowns + row counter when df_state is updated."""
            if df is None or isinstance(df, bool):
                return (
                    gr.update(choices=[]),
                    gr.update(choices=[]),
                    gr.update(choices=[]),
                    "**Current Rows:** No data loaded",
                )
        ...
        def update_viz_dropdowns(df):
            """Update visualization dropdowns when df_state is updated."""
            if df is None or isinstance(df, bool):
                return (
                    gr.update(choices=[]),  # ts_date_col
                    gr.update(choices=[]),  # dist_col
                    gr.update(choices=[]),  # cat_col
                    gr.update(choices=["None"]),  # cat_value_col
                    gr.update(choices=[]),  # scatter_x
                    gr.update(choices=[]),  # scatter_y
                    gr.update(choices=["None"]),  # scatter_color
                    gr.update(choices=[]),  # ts_value_col
                )
            ...
            return (
                ...
                gr.update(choices=num_cols),  # dist_col
                gr.update(choices=cat_cols),  # cat_col
                gr.update(choices=["None"] + all_cols),  # cat_value_col
                gr.update(choices=num_cols),  # scatter_x
                gr.update(choices=num_cols),  # scatter_y
                gr.update(choices=["None"] + all_cols),  # scatter_color
                gr.update(choices=num_cols),  # ts_value_col
            )

        def update_export_dropdowns(df):
            """Update export tab dropdowns when df_state is updated."""
            if df is None or isinstance(df, bool):
                return (
                    gr.update(choices=[]),  # export_column
                    gr.update(choices=[], visible=False),  # export_date_col
                )
        ...
        # ====================================================================
        # MAIN UPLOAD BUTTON CHAIN (central place where df_state changes)
        # ====================================================================
        ...
        upload_btn.click(
            ...
            fn=update_export_dropdowns,
            inputs=[df_state],
            outputs=[export_column, export_date_col],
        )
